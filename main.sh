...
echo "Would you like to continue? yes/no: " && read confirm01

if [[ "$(whoami)" = "root" ]]; then
	echo "Running as root, good job!"
else
	echo "Please run me as root (sudo/doas)"
fi
...
echo "Which user will be using Looking Glass? " && read user

if [[ -d "/home/$user" ]]; then
	echo "$user is valid."
else
	echo "$user is not valid." ; exit
fi
...
if [[ "$init" = "" ]]; then
	echo "Your init system is not compatible. Sorry. If you want, you can contribute with support for more."  && exit 3
fi
...
echo "What version of Looking Glass would you like to set up? This must be the same as your Windows VM: Available: 'stable', 'rc', 'bleeding': " && read lgver
...
echo "Are you sure you wanna set up Looking Glass? 'Y/N': " && read confirm02
...
if [[ "$distro" = "gentoo" ]]; then
	tput setaf 1; echo "WARNING: You are running a Gentoo based distribution. Expect compile times to potentially be high."
...
$installcommand || echo "Failed to install. Exiting." ; exit 4
